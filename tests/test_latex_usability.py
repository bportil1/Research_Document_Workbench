...
class LatexFrontendContractTests(unittest.TestCase):
    def test_directory_context_diagnostics_and_navigation_controls_are_present(self):
        ...
        self.assertIn("LIVE_PREVIEW_STORAGE_KEY", js)
        self.assertIn("VIEW_MODE_STORAGE_KEY", js)
        self.assertIn("persistLivePreviewEnabled", js)
        self.assertIn("restoreLivePreviewState", js)
        self.assertIn("persistViewMode", js)
        self.assertIn("restoreViewMode", js)
        self.assertIn('setView(readStoredViewMode(), { persist: false })', js)
        self.assertIn('id="openEditorWindowBtn"', html)
        self.assertIn('id="detachedModeBadge"', html)
        self.assertIn('startupParams.get("detached")', js)
        self.assertIn("openDetachedEditorWindow", js)
        self.assertIn('"popup=yes,width=1200,height=900"', js)
        self.assertIn('url.searchParams.set("project", currentProject)', js)
        self.assertIn('url.searchParams.set("file", currentFile)', js)
        self.assertIn('setView("editor", { persist: false })', js)
        self.assertIn('loadProjects(detachedProject || undefined, detachedFile || undefined)', js)
        self.assertIn("body.detached-editor-mode .workspace", css)
        self.assertIn('body.detached-editor-mode [data-view="preview"]', css)
        self.assertIn(".detached-mode-badge", css)
        self.assertIn('id="openPreviewWindowBtn"', html)
        self.assertIn("isDetachedPreview", js)
        self.assertIn("configureDetachedPreviewMode", js)
        self.assertIn("openDetachedPreviewWindow", js)
        self.assertIn("detachedPreviewUrl", js)
        self.assertIn("applyDetachedPreviewSnapshot", js)
        self.assertIn("sameOriginPreviewUrl", js)
        self.assertIn('"rdw-detached-preview"', js)
        self.assertIn('"popup=yes,width=1050,height=900"', js)
        self.assertIn('setView("preview", { persist: false })', js)
        self.assertIn("body.detached-preview-mode .workspace", css)
        self.assertIn("body.detached-preview-mode #textToolbar", css)
        self.assertIn("body.detached-preview-mode .editor-grid.preview .preview-pane", css)
        self.assertIn("DOCUMENT_SESSION_CHANNEL_NAME", js)
        self.assertIn('new BroadcastChannel(DOCUMENT_SESSION_CHANNEL_NAME)', js)
        self.assertIn('postDocumentSessionEvent("source-dirty"', js)
        self.assertIn('postDocumentSessionEvent("source-saved"', js)
        self.assertIn('message.type === "compile-succeeded"', js)
        self.assertIn("applyRemoteSavedSource", js)
        self.assertIn("applyRemoteCompileSuccess", js)
        self.assertIn("documentSessionMatches", js)
        self.assertIn('message.source === documentSessionInstanceId', js)
        self.assertIn('postDocumentSessionEvent("editor-claimed")', js)
        self.assertIn('postDocumentSessionEvent("editor-released")', js)
        self.assertIn("applyRemoteEditorClaim", js)
        self.assertIn("editor.readOnly = true", js)
        self.assertIn('editor.addEventListener("pointerdown"', js)
        self.assertIn("Compile paused because another Workbench window owns editing", js)
        self.assertIn("Focus this editor to take control", js)
        self.assertIn("local unsaved edits were kept", js)
